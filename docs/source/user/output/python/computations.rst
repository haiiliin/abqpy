...
The FieldOutput operations are significantly more efficient than the FieldValue operators. You can save the computed FieldOutput objects with the following procedure (see the example, :doc:`user/output/python/example-scripts:computations with FieldOutput objects`):
...
The steps required to transform results are (see also the example :doc:`user/output/python/example-scripts:transformation of field results`):
