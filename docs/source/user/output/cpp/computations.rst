...
The FieldOutput operations are significantly more efficient than the FieldValue operators. You can save the computed FieldOutput objects with the following procedure (see the example, :doc:`../python/examples:computations with FieldOutput objects`):
...
The steps required to transform results are (see also the example :doc:`../python/examples:transformation of field results`):
