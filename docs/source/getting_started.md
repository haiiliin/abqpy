...
````{tab} pip
```{code-block} shell
pip install -U abqpy==2016.*
pip install -U abqpy2016
```
````

````{tab} conda
```{code-block} shell
conda install conda-forge::abqpy=2016
```
````

````{tab} Source
```{code-block} shell
pip install git+https://github.com/haiiliin/abqpy@2016
```
````

````{tab} Jupyter
```{code-block} shell
pip install -U abqpy[jupyter]==2016.*
pip install -U abqpy2016[jupyter]
...
- Use the `abaqus` command in the command line:
  ...
  ```
  See [here](https://help.3ds.com/2016/English/DSSIMULIA_Established/SIMACAEEXCRefMap/simaexc-c-caeproc.htm?contextscope=all)
