...
class AbqpyCLI(AbqpyCLIBase):
    ...
    def cae(
        ...
        *args,
        ...
        """
        # Parse options
        script = f'"{script.strip('"')}"' if " " in script else script
    ...
    def pde(self, *scripts: str, script: str | None = None, **options):
        ...
        """
        script = f'"{script.strip('"')}"' if script is not None and " " in script else script
        scripts = tuple(f'"{s.strip('"')}"' if " " in s else s for s in scripts)
